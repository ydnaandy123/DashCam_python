#!/usr/bin/python3
# ==============================================================
# viewpoint systhesis use OpenGL draw texture each frame
# ==============================================================
import numpy as np
import scipy.misc
...
needVisual, needGround, addPlane = False, False, False
# Novel view
imageSynthesis, planeJoints, needTexture, needSuccessive = True, False, True, True
offsetPos, offsetDeg = np.array([0, 5.0, 0]), 0
startX, startY = 0.367, 11.0
endX, endY = 0.616, 18.0
slop = 30
texture_height, texture_width = 256, 512  # [416, 832]
"""
Variables
"""
...
for fileIndex in range(sleIndex, sleIndex+1):
    ...
    if createSV:
        ...
        sv3DRegion.create_region_time_bilinear_depth(start=7, end=10)
        # sv3DRegion.create_region_time_bilinear_depth()
        ...
        if imageSynthesis:
            for curPano in range(0, pano_length-1):
                for curSlop in range(0, slop):
                    print('{:d}/{:d} {:d}/{:d}'.format(curSlop, slop, curPano, pano_length))
                    offsetPos = pano_ori_vec[curPano] + curSlop*(pano_ori_vec[curPano+1]-pano_ori_vec[curPano])/slop
                    print(offsetPos)
                    dis_vec = np.array(pano_ori_vec - offsetPos)
                            ...
                            if addPlane:
                                for j in range(0, pano_length):
                                    ...
                                    for i in range(0, len(sv3D.all_plane)):
                                        programGround = glumpy_setting.ProgramPlane(data=sv3D.all_plane[i]['data'],
                                                                                    name='test',
                    ...
                    if needTexture:
                        gpyViewWindow = glumpy_setting.GpyViewWindowSingle(width=texture_width, height=texture_height)
                        gpyViewWindow.set_program(programTextureSingle)
                        gpyViewWindow.set_idx(nearest_pano_idx, curSlop)
                        gpyViewWindow.run()
