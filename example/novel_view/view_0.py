...
for fileIndex in range(sleIndex, sleIndex+1):
    ...
    if createSV:
        ...
        sv3DRegion.create_region_time(start=5, end=12)
        length = len(sv3DRegion.panoramaList)
        anchor_inv = np.linalg.inv(sv3DRegion.anchorMatrix)
        # TODO numpy array
        zero_vec, pano_ori_set, dis_len_set = [0, 0, 0, 1], np.zeros((length, 3)), np.zeros((length))
        ...
        #dis_len_set = np.ndarray.sort(dis_len_set)
        ...
        programSV3DTopology = glumpy_setting.ProgramSV3DTopology(
            ...
            anchor_matrix=sv3DRegion.anchorMatrix, anchor_yaw=sv3DRegion.anchorYaw, is_inverse=needMatchInfo3d)
        data = np.zeros(length+1, dtype=[('a_position', np.float32, 3), ('a_color', np.float32, 3)])
        data['a_color'] = [0, 1, 0]
