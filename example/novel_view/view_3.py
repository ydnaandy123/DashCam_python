    ...
    if createSV:
        ...
        for i in range(0, pano_length):
            ...
            if imageSynthesis and i == nearest_pano_idx:
                ...
                pp_xyz = []
                plane_pano = np.zeros((256, 256, 3), dtype=np.float32)
                for ppx in range(0, 256):
                    for ppy in range(0, 256):
                        x, y, z = ppx-64, 50, -ppy+128
                        pp_xyz.append([x, y, z])
                        if y == 0:
                            continue
                        lng, lat = base_process.pos_2_deg(x, y, z)
                ...
                data = np.zeros((256 * 256), dtype=[('a_position', np.float32, 3), ('a_color', np.float32, 3)])
                data['a_color'] = np.reshape(plane_pano, (256 * 256, 3))
                data['a_position'] = np.array(pp_xyz)
                data_y = np.reshape(data['a_position'], (256 * 256, 3))
