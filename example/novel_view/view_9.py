#!/usr/bin/python3
# ==============================================================
# viewpoint systhesis perspective inverse mapping
# ==============================================================
import numpy as np
import sys
import scipy.misc
...
for fileIndex in range(sleIndex, sleIndex+1):
    ...
    if createSV:
        ...
        for i in range(0, pano_length):
            ...
            for degree in range(0, 360, 90):
                ...
                for p_y in range(0, perspective_height):
                    for p_x in range(0, perspective_width):
                        ...
                        y = perspective_width/2
                        lng, lat = base_process.pos_2_deg(x, y, z)

                        lng = (lng + degree + randomDeg) % 360
                        img_x = np.round(lng / 360.0 * pano_width)
                        img_y = np.round(-(lat - 90) / 180.0 * pano_height)

                        perspective_90[p_y, p_x, :] = ori_pano[img_y, img_x, :]
            ...
            scipy.misc.imsave('yo.png', perspective_90_visual)
